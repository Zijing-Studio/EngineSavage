...
- 世界 (World) ：整个游戏对应一个世界，世界是对整个游戏的封装
- 单元 (Unit) ：游戏中所有可以交互的单位称为单元，例如游戏中的英雄
- 模板 (Template) ：单元的种类，类比于c++中声明一个自定义类，而单元是一个实例化的自定义类，每个单元都绑定到一个模板上
- 事件 (event) ：事件是指交互过程中传递的信息，存在事件的发出者和事件的接收者，信息包含事件的种类，事件的发出者是谁等等
- 事件的发出者 (sender) ：事件的发出者可以是单元也可以是世界
- 事件的接收者 (receiver) ：事件的接收者可以是单元，模板，也可以是世界
- 事件过滤器 (event filter) ：实现方式为模板的成员函数，每个事件过滤器只针对某种事件作出响应，响应可以是修改成员变量，发起新的事件

每当有新的事件产生时，就向世界中所有的单元和模板广播这个事件（可能该事件为实例化某个模板形成新的单元，因此需要向模板广播），遍历其绑定的所有的事件过滤器，如果某事件过滤器可以对该事件进行应答，那么马上执行响应。

**事件的连锁：在具体实现过程中，lzj认为不需要有事件实体，采取函数调用的方式来实现事件框架，因此所有的事件都是没有延迟马上执行响应的。例如，对某点造成范围伤害，该范围内两角色A和B都需要对伤害事件e1进行响应和掉血，如果先遍历A，那么A的掉血如果触发新的事件e2，优先处理e2，再执行B对e1的响应。**

**存在问题：如上例子中，先遍历A与先遍历B结果可能不等价，例如B拥有一旦掉血就传送走的技能，A拥有一旦掉血就对周围角色造成致死伤害的技能**
